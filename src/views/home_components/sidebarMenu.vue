<template>
    <Menu ref="sideMenu" :active-name="currentPageName" :open-names="openedSubmenuArr" :theme="$store.state.app.menuTheme" width="auto" @on-select="changeMenu">
        <template v-for="item in menuList">
            <MenuItem v-if="item.children.length<=1" :name="item.children[0].name" :key="item.path">
                <Icon :type="item.icon" :size="iconSize" :key="item.path"></Icon>
                <span class="layout-text" style="font-weight: bold">{{ item.title }}</span>
            </MenuItem>
        ...
        changeMenu (active) {
            if (active !== 'accesstest_index') {
                ...
                if (!tagHasOpened) {
                    ...
                    localStorage.pageOpenedList = JSON.stringify(this.$store.state.app.pageOpenedList); // 本地存储已打开页面
                }
