...
gantt._eventable = function (obj) {
  ...
  obj.attachEvent = function (name, catcher, callObj) {
    ...
    if (!this[name]) this[name] = new this._eventCatcher(callObj || this);
  ...
  obj._eventCatcher = function (obj) {
    ...
    z.addEvent = function (ev) {
      if (typeof ev == "function") return dhx_catch.push(ev) - 1;
      return false;
    };
  ...
  obj.detachAllEvents = function () {
    for (var name in this) {
      if (name.indexOf("ev_") === 0) delete this[name];
    }
  };
  obj = null;
};
...
gantt.copy = function (object) {
  ...
  if (object && (typeof object === "undefined" ? "undefined" : _typeof(object)) == "object") {
    ...
    for (i in object) {
      if (Object.prototype.hasOwnProperty.apply(object, [i])) result[i] = gantt.copy(object[i]);
    }
  }
...
(function () {
  ...
  gantt.mixin(gantt.config, {
    ...
    lightbox: {
      sections: [{ name: "description", height: 70, map_to: "text", type: "textarea", focus: true }, { name: "time", type: "duration", map_to: "auto" }],
      project_sections: [{ name: "description", height: 70, map_to: "text", type: "textarea", focus: true }, { name: "type", type: "typeselect", map_to: "type" }, { name: "time", type: "duration", readonly: true, map_to: "auto" }],
      milestone_sections: [{ name: "description", height: 70, map_to: "text", type: "textarea", focus: true }, { name: "type", type: "typeselect", map_to: "type" }, { name: "time", type: "duration", single_date: true, map_to: "auto" }]
    },
    drag_lightbox: true,
    sort: false,
  ...
  gantt._init_templates = function () {
    ...
    gantt.mixin(this.templates, {
      ...
      task_cell_class: function task_cell_class(item, date) {
        return "";
      },
      ...
      date_grid: function date_grid(date, item) {
...
gantt._DnD.prototype = {
  traceDragEvents: function traceDragEvents(domElement, inputMethod) {
    ...
    var limited_mousemove = gantt.bind(function (e) {
      ...
      if (gantt.defined(this.config.updates_per_second)) {
        if (!gantt._checkTimeout(this, this.config.updates_per_second)) return true;
      }
      return mousemove(e);
    }, this);
  ...
  dragStart: function dragStart(obj, e, inputMethod) {
    ...
    if (this._settings) gantt.mixin(this.config, this._settings, true);
...
gantt._init_grid = function () {
  ...
  this._click.gantt_row = this.bind(function (e, id, trg) {
    if (id !== null) {
      ...
      if (this.config.scroll_on_click) this.showDate(task.start_date);
  ...
  this._click.gantt_grid_head_cell = this.bind(function (e, id, trg) {
    ...
    if (!this.callEvent("onGridHeaderClick", [column, e])) return;
...
gantt._render_grid_header = function () {
  ...
  for (var i = 0; i < columns.length; i++) {
    ...
    if (last && this._get_grid_width() > width + colWidth) col.width = colWidth = this._get_grid_width() - width;
    width += colWidth;
    var sort = this._sort && col.name == this._sort.name ? "<div class='gantt_sort gantt_" + this._sort.direction + "'></div>" : "";
    var cssClass = ["gantt_grid_head_cell", "gantt_grid_head_" + col.name, last ? "gantt_last_cell" : "", this.templates.grid_header_class(col.name, col)].join(" ");
...
gantt._render_grid_item = function (item) {
  ...
  for (var i = 0; i < columns.length; i++) {
    ...
    if (col.name == "add") {
      ...
      if (value instanceof Date) value = this.templates.date_grid(value, item);
      textValue = value;
    ...
    if (this.defined(col.align)) style += "text-align:" + col.align + ";";
...
gantt.moveTask = function (sid, tindex, parent) {
   ...
   */
  if (this.callEvent("onBeforeTaskMove", [sid, parent, tindex]) === false) return;
  ...
  if (this.callEvent("onAfterTaskMove", [sid, parent, tindex]) === false) return;
...
gantt._init_dnd = function () {
  ...
  if (this.defined(this.config.dnd_sensitivity)) dnd.config.sensitivity = this.config.dnd_sensitivity;
  ...
  dnd.attachEvent("onBeforeDragStart", this.bind(function (obj, e) {
    ...
    if (gantt._is_readonly(task)) return false;
  ...
  dnd._getTaskByY = this.bind(function (y, dropIndex) {
    ...
    if (index > this._order.length - 1) return null;
        ...
        if (next) {
          if (next.id != item.id) {
            ...
            if (this.config.order_branch_free) {
              if (!(this.isChildOf(item.id, over.id) && this.getChildren(over.id).length == 1)) return;else {
...
(function () {
  ...
  gantt._has_children = function (id) {
    ...
    if (old_has_children.apply(this, arguments)) return true;
...
gantt._refresh_on_toggle_element = function (id) {
  ...
  if (!gantt._need_dynamic_loading(id) || !gantt.getTask(id).$open) this.refreshData();
};
...
(function () {
  ...
  gantt._waiAria = {
    ...
    _taskCommonAttr: function _taskCommonAttr(task, div) {
      ...
      div.setAttribute("aria-selected", gantt.getState().selected_task == task.id || gantt.isSelectedTask && gantt.isSelectedTask(task.id) ? "true" : "false");
    },
    ...
    gridCellAttrString: function gridCellAttrString(column, textValue) {
...
gantt._scale_helpers = {
  ...
  prepareConfigs: function prepareConfigs(scales, min_coll_width, container_width, scale_height) {
  ...
  initScaleConfig: function initScaleConfig(config) {
    var cfg = gantt.mixin({
      count: 0,
      col_width: 0,
      full_width: 0,
  ...
  eachColumn: function eachColumn(unit, step, callback) {
    ...
    while (+curr < +end) {
      ...
      if (gantt.date[unit + '_start']) curr = gantt.date[unit + "_start"](curr);
    }
  },
...
gantt._tasks_dnd = {
  ...
  _resize: function _resize(ev, shift, drag) {
    ...
    if (ev.end_date - ev.start_date < cfg.min_duration) {
      if (drag.left) ev.start_date = gantt.calculateEndDate({ start_date: ev.end_date, duration: -1, task: ev });else ev.end_date = gantt.calculateEndDate({ start_date: ev.start_date, duration: 1, task: ev });
    }
    gantt._init_task_timing(ev);
  },
  _resize_progress: function _resize_progress(ev, shift, drag) {
    var coords_x = this._drag_task_coords(ev, drag);

    var diff = Math.max(0, drag.pos.x - coords_x.start);
    ev.progress = Math.min(1, diff / (coords_x.end - coords_x.start));
  },
  _move: function _move(ev, shift, drag) {
    var coords_x = this._drag_task_coords(ev, drag);
  ...
  on_mouse_down: function on_mouse_down(e, src) {
    ...
    if (!drag) {
      ...
      if (drag.mode && drag.mode != gantt.config.drag_mode.ignore && gantt.config["drag_" + drag.mode]) {
        ...
        if (gantt._is_flex_task(task) && drag.mode != gantt.config.drag_mode.progress) {
  ...
  _get_drag_mode: function _get_drag_mode(className) {
...
gantt._render_link_element = function (link) {
  ...
  if (this.config.highlight_critical_path && this.isCriticalLink) {
    if (this.isCriticalLink(link)) css += " gantt_critical_link";
  }
...
gantt._drawer = {
  ...
  get_wrapper_sizes: function get_wrapper_sizes(v) {
    var res,
  ...
  get_line_sizes: function get_line_sizes(v) {
    var res,
        line_size = gantt.config.link_line_width,
        wrapper_size = gantt.config.link_wrapper_width,
  ...
  render_line: function render_line(v) {
...
gantt._path_builder = {
  ...
  point_to: function point_to(direction, diff, point) {
    if (!point) point = gantt.copy(this.point());else point = { x: point.x, y: point.y };
  ...
  get_points: function get_points(link) {
  ...
  get_endpoint: function get_endpoint(link) {
...
gantt._init_links_dnd = function () {
  ...
  dnd.attachEvent("onBeforeDragStart", gantt.bind(function (obj, e) {
    ...
    resetDndState();
    if (gantt.getState().drag_id) return false;

    if (gantt._locate_css(target, link_edge_marker)) {
      if (gantt._locate_css(target, start_marker)) gantt._link_source_task_start = true;
  ...
  dnd.attachEvent("onDragEnd", gantt.bind(function () {
    ...
    if (drag.from && drag.to && drag.from != drag.to) {
      ...
      if (link.type && gantt.isLinkAllowed(link)) gantt.addLink(link);
    }
    ...
    if (this.config.touch) {
      ...
      if (drag.from) gantt.refreshTask(drag.from, false);
      if (drag.to) gantt.refreshTask(drag.to, false);
    }
    removeDirectionLine();
  }, this));
...
gantt._task_renderer = function (id, render_one, node, filter) {
  ...
  if (!this._task_area_pulls) this._task_area_pulls = {};

  if (!this._task_area_renderers) this._task_area_renderers = {};

  if (this._task_area_renderers[id]) return this._task_area_renderers[id];

  if (!render_one) this.assert(false, "Invalid renderer call");

  if (node) node.setAttribute(this.config.layer_attribute, true);

  this._task_area_renderers[id] = {
    render_item: function render_item(item, container) {
...
gantt._init_tasks = function () {
  ...
  this._dbl_click.gantt_link_point = this.bind(function (e, id, trg) {
    ...
    if (link) this._delete_link_handler(link, e);
    return false;
  }, this);
  ...
  if (this._onTaskIdChange) this.detachEvent(this._onTaskIdChange);
  ...
  if (this._onLinkIdChange) this.detachEvent(this._onLinkIdChange);
...
gantt._filter_link = function (id, link) {
  ...
  if (!(gantt.isTaskVisible(link.source) && gantt.isTaskVisible(link.target)) || gantt._isAllowedUnscheduledTask(gantt.getTask(link.source)) || gantt._isAllowedUnscheduledTask(gantt.getTask(link.target))) return false;
...
gantt._update_layout_sizes = function () {
  ...
  for (var i = 0, len = data_els.length; i < len; i++) {
    var el = data_els[i];
    if (this._is_layer(el) && el.style) el.style.width = cfg.full_width + "px";
  }
  ...
  for (var i = startIndex; i < endIndex; i++) {
    ...
    if (width) {
      ...
      if (newValue.length > 4) {
        ...
        val = newValue[1] + '' + newValue[0] + '号 ' + newValue[2] + ' ' + newValue[4] + '' + newValue[3] + '号 (第' + newValue[6] + '' + newValue[5] + ')';
      } else {
        val = value;
      }
  ...
  if (gantt.config.show_task_cells) {
    for (var j = 0; j < count; j++) {
      ...
      if (width > 0) {
        ...
        if (cssTemplate) cssclass += " " + cssTemplate;
...
gantt._combine_item_class = function (basic, template, itemId) {
  ...
  if (this._is_flex_task(task)) css.push("gantt_dependent_task");

  if (this.config.select_task && itemId == state.selected_task) css.push("gantt_selected");
  ...
  if (links.link_source_id == itemId) css.push("gantt_link_source");

  if (links.link_target_id == itemId) css.push("gantt_link_target");

  if (this.config.highlight_critical_path && this.isCriticalTask) {
    if (this.isCriticalTask(task)) css.push("gantt_critical_task");
  }
...
gantt._render_pair = function (parent, css, task, content) {
  ...
  if (+task.start_date >= +state.min_date) parent.appendChild(content(css + " task_left"));

  if (+task.end_date <= +state.max_date) parent.appendChild(content(css + " task_right"));
};

gantt._get_task_height = function () {
  // height of the bar item
  var height = this.config.task_height;
  if (height == "full") height = this.config.row_height - 5;
...
gantt._task_default_render = function (task) {
  if (this._isAllowedUnscheduledTask(task)) return;
  ...
  var styles = ["left:" + pos.x + "px", "top:" + (padd + pos.y) + 'px', "height:" + height + 'px', "line-height:" + Math.max(height < 30 ? height - 2 : height, 0) + 'px', "width:" + width + 'px'];
  if (task.color) {
...
gantt._render_task_content = function (task, width) {
  var content = document.createElement("div");
  if (this._get_safe_type(task.type) != this.config.types.milestone) content.innerHTML = this.templates.task_text(task.start_date, task.end_date, task);
...
gantt.dateFromPos = function (x) {
  ...
  var part = 0;
  if (col_width) part = (x - summ) / col_width;
...
gantt.posFromDate = function (date) {
  ...
  if (wholeCells == gantt._tasks.width.length) pos += gantt._tasks.width[gantt._tasks.width.length - 1];
...
gantt._day_index_by_date = function (date) {
  ...
  if (pos <= this._min_date) return 0;

  if (pos >= this._max_date) return days.length;
...
gantt._layers = {
  prepareConfig: function prepareConfig(config) {
    ...
    if (!config.container) config.container = document.createElement("div");

    return config;
  },
  create: function create(get_container, rel_root) {
    return {
      tempCollection: [],
      renderers: {},
      ...
      getRenderer: function getRenderer(id) {
        return this.renderers[id];
      },
      add: function add(layer) {
        if (layer) this.tempCollection.push(layer);
        ...
        for (var i = 0; i < pending.length; i++) {
          ...
          if (!node.parentNode) {
            ...
            if (topmost) {
              ...
              if (rel) container.insertBefore(node, rel);else container.appendChild(node);
            }
          }
...
gantt._build_pull = function (tasks) {
  ...
  for (var i = 0, len = tasks.length; i < len; i++) {
    task = tasks[i];
    if (this._load_task(task)) loaded.push(task);
  }
  return loaded;
};
...
gantt._sync_order_item = function (item, hidden) {
  if (item.id !== gantt.config.root_id) {
    //do not trigger event for virtual root

    this._order_full.push(item.id);
    if (!hidden && this._filter_task(item.id, item) && this.callEvent("onBeforeTaskDisplay", [item.id, item])) {
...
gantt.getGlobalTaskIndex = function (id) {
  ...
  if (index !== undefined) return index;else return -1;
};
...
gantt.json = {
  parse: function parse(data) {
    ...
    if (typeof data == "string") {
      if (window.JSON) data = JSON.parse(data);else {
...
gantt._calendars.calendarArgumentsHelper = {
  ...
  getClosestWorkTimeArguments: function getClosestWorkTimeArguments(config) {
    config = arguments[0];
  ...
  getDurationConfig: function getDurationConfig(start, end, task, calendar) {
...
gantt._calendars.calendarStrategy = {
  ...
  _workingUnitsCache: {
    ...
    get: function get(unit, date) {
      ...
      if (cache && cache[unit]) {
        ...
        if (units[time] !== undefined) result = units[time];
      }
      return result;
    },

    put: function put(unit, date, value) {
  ...
  _checkIfWorkingUnit: function _checkIfWorkingUnit(date, unit, order) {
    if (order === undefined) {
    ...
    if (order) {
      ...
      if (!this._isWorkTime(date, this.units[order - 1], order - 1)) return false;
    }
  ...
  _is_work_hour: function _is_work_hour(date) {
    ...
    for (var i = 0; i < hours.length; i += 2) {
      if (hours[i + 1] === undefined) {
        ...
        if (hour >= hours[i] && hour < hours[i + 1]) return true;
      }
    }
    return false;
  },
  ...
  _getHoursPerDay: function _getHoursPerDay(date) {
  ...
  _getWorkHoursForRange: function _getWorkHoursForRange(from, to) {
    var hours = 0;
    var start = new Date(from),
        end = new Date(to);

    while (start.valueOf() < end.valueOf()) {
      if (this._isWorkTime(start, "day")) hours += this._getHoursPerDay(start);
      start = this._nextDate(start, "day", 1);
    }
    return hours;
  },
  _getWorkUnitsBetweenHours: function _getWorkUnitsBetweenHours(from, to, unit, step) {
    var start = new Date(from),
        end = new Date(to);
    step = step || 1;
  ...
  _setCalendar: function _setCalendar(settings) {
  ...
  _isEmptyCalendar: function _isEmptyCalendar(settings) {
    var result = false,
        datesArray = [],
        isFullWeekSet = true;
    ...
    for (var i = 0; i < 7; i++) {
      if (checkFullArray[i] != i) isFullWeekSet = false;
    }
    if (isFullWeekSet) return !result;
  ...
  _getWorkHours: function _getWorkHours(date) {
  ...
  _isWorkTime: function _isWorkTime(date, unit, order) {
  ...
  _calculateEndDate: function _calculateEndDate(from, duration, unit, step) {
    ...
    while (added < duration) {
      ...
      if (this._isWorkTime(step > 0 ? new Date(next.valueOf() - 1) : new Date(next.valueOf() + 1), unit)) added++;
      start = next;
    }
    return start;
  },
  ...
  _getClosestWorkTime: function _getClosestWorkTime(settings) {
    if (this._isWorkTime(settings.date, settings.unit)) return settings.date;
...
(function () {
  var workingUnitsCache = function workingUnitsCache() {
    this._cache = {};
  };
...
gantt._calendars.calendarManager = {
  _calendars: {},
  _getDayHoursForMultiple: function _getDayHoursForMultiple(calendars, date) {
    var units = [],
        tick = true,
        currPos = 0,
        is_work_hour = false,
  ...
  mergeCalendars: function mergeCalendars() {
    ...
    for (var i = 0; i < calendars.length; i++) {
      for (var value in calendars[i].worktime.dates) {
        if (+value > 10000) {
  ...
  addCalendar: function addCalendar(calendar) {
    ...
    if (!gantt.config.worktimes) gantt.config.worktimes = {};
    gantt.config.worktimes[calendar.id] = calendar.worktime;
    return calendar.id;
  },

  deleteCalendar: function deleteCalendar(calendar) {
    if (calendar && this._calendars[calendar.id]) delete this._calendars[calendar.id];
    if (gantt.config.worktimes && gantt.config.worktimes[calendar.id]) delete gantt.config.worktimes[calendar.id];
  },
...
gantt._timeCalculator = {
  ...
  setWorkTime: function setWorkTime(config, calendar) {
    ...
    if (!calendar) calendar = gantt._calendars.calendarManager.getCalendar(); // Global
...
gantt.isTaskVisible = function (id) {
  if (!this._pull[id]) return false;

  var task = this._pull[id];
  if (!(+task.start_date < +this._max_date && +task.end_date > +this._min_date || gantt._isAllowedUnscheduledTask(task))) return false;
...
gantt._add_branch = function (task, index) {
  ...
  if (!this.hasChild(pid)) this._branches[pid] = [];
...
gantt._getChildLinks = function (id) {
  ...
  links = [];
  for (var i in res) {
    if (this.isLinkExists(i)) links.push(i);
  }
...
gantt._deleteRelatedLinks = function (links, silent) {
  ...
  if (use_dp) {
    ...
    if (send_changes) this._dp.sendAllData();
  }
};
...
gantt._init_task = function (task) {
  if (!gantt.defined(task.id)) task.id = gantt.uid();

  if (task.start_date) task.start_date = gantt.date.parseDate(task.start_date, "xml_date");
  ...
  if (task.start_date && task.end_date) gantt.correctTaskWorkTime(task);
...
gantt.getSubtaskDates = function (task_id) {
  ...
  this.eachTask(function (child) {
    if (this._get_safe_type(child.type) == gantt.config.types.project || this.isUnscheduledTask(child)) return;
...
gantt.isChildOf = function (child_id, parent_id) {
  if (!this.isTaskExists(child_id)) return false;
  if (parent_id === this.config.root_id) return this.isTaskExists(child_id);
  ...
  while (task && this.isTaskExists(pid)) {
    ...
    if (task && task.id == parent_id) return true;
...
gantt.getNext = function (id) {
  for (var i = 0; i < this._order.length - 1; i++) {
    if (this._order[i] == id) return this._order[i + 1];
  }
  return null;
};
...
gantt.getNextSibling = function (id) {
  ...
  for (var i = 0, len = siblings.length; i < len; i++) {
    if (siblings[i] == id) return siblings[i + 1] || null;
  }
  return null;
};
...
gantt._dp_init = function (dp) {
  ...
  dp._getRowData = gantt.bind(function (id, pref) {
    var task;
    if (dp._ganttMode == "tasks") task = this.isTaskExists(id) ? this.getTask(id) : { id: id };else task = this.isLinkExists(id) ? this.getLink(id) : { id: id };
    ...
    for (var key in task) {
      ...
      var value = task[key];
      if (value instanceof Date) data[key] = this.templates.xml_format(value);else if (value === null) data[key] = "";else data[key] = value;
    }
  ...
  this._change_id = gantt.bind(function (oldid, newid) {
    if (dp._ganttMode != "tasks") this.changeLinkId(oldid, newid);else this.changeTaskId(oldid, newid);
  }, this);

  this._row_style = function (row_id, classname) {
    if (dp._ganttMode != "tasks") return;
    if (!gantt.isTaskExists(row_id)) return;
...
gantt._init_link = function (link) {
  if (!gantt.defined(link.id)) link.id = gantt.uid();
  return link;
};
...
gantt._sync_links = function () {
  ...
  for (var id in this._lpull) {
    ...
    if (this._pull[link.source]) this._pull[link.source].$source.push(id);
    if (this._pull[link.target]) this._pull[link.target].$target.push(id);
  }
};
...
gantt.updateLink = function (id, data) {
  if (!gantt.defined(data)) data = this.getLink(id);
...
gantt._get_skin = function (force) {
  ...
  if (config[1] && typeof config[1].width == "undefined") config[1].width = skinset._second_column_width;
  if (config[2] && typeof config[2].width == "undefined") config[2].width = skinset._third_column_width;
...
gantt.getLightbox = function (type) {
  ...
  if (!this._lightbox || this.getLightboxType() != this._get_safe_type(type)) {
    ...
    if (gantt.config.wide_form || full_width) d.className += " gantt_cal_light_wide";
...
gantt.getLightboxValues = function () {
  ...
  for (var i = 0; i < sns.length; i++) {
    ...
    if (typeof map_to == "string" && map_to != "auto") {
      task[map_to] = res;
    } else if ((typeof map_to === "undefined" ? "undefined" : _typeof(map_to)) == "object") {
...
gantt._set_lightbox_values = function (data, box) {
  ...
  if (data.id) gantt._lightbox_id = data.id;
};
...
gantt.form_blocks = {
  ...
  _fill_lightbox_select: function _fill_lightbox_select(s, i, d, map, cfg) {
  ...
  time: {
    ...
    get_value: function get_value(node, ev, config) {
      ...
      if (end_date <= start_date) end_date = gantt.date.add(start_date, gantt._get_timepicker_step(), "minute");
  ...
  duration: {
    render: function render(sns) {
      ...
      var duration = "<div class='gantt_duration' " + singleDate + ">" + "<input type='button' class='gantt_duration_dec' value='−'" + readonly + ">" + "<input type='text' value='5' class='gantt_duration_value'" + readonly + " " + ariaAttr + ">" + "<input type='button' class='gantt_duration_inc' value='+'" + readonly + "> " + label + " <span></span>" + "</div>";
    ...
    _get_start_date: function _get_start_date(node, config) {
      var s = node.getElementsByTagName("select");
      var map = config._time_format_order;
      var hours = 0;
      var minutes = 0;
    ...
    _get_duration: function _get_duration(node, config) {
  ...
  parent: {
    _filter: function _filter(options, config, item_id) {
    ...
    _display: function _display(config, item_id) {
      var tasks = [],
          options = [];
      if (item_id) {
...
gantt.dataProcessor.prototype = {
  ...
  _clearUpdateFlag: function _clearUpdateFlag(id) {},
  ...
  set_invalid: function set_invalid(id, mode, details) {
    if (details) mode = {
      value: mode, details: details, toString: function toString() {
  ...
  serialize: function serialize(data, id) {
    ...
    if (typeof id != "undefined") return this.serialize_one(data, "");else {
      ...
      if (gantt.security_key) stack.push("dhx_security=" + gantt.security_key);
      return stack.join("&");
    }
  },
  ...
  _sendData: function _sendData(a1, rowId) {
    ...
    var that = this;
    var back = function back(xml) {
      var ids = [];
      if (rowId) ids.push(rowId);else if (a1) for (var key in a1) {
        ids.push(key);
  ...
  sendAllData: function sendAllData() {
    ...
    if (this._tSend) this._sendData(this._getAllData());else for (var i = 0; i < this.updatedRows.length; i++) {
   ...
   *    @type: public
   */
  setVerificator: function setVerificator(ind, verifFunction) {
   ...
   *    @type: public
   */
  clearVerificator: function clearVerificator(ind) {
  ...
  enableDebug: function enableDebug(mode) {},
...
gantt._insert_callback = function (upd, id, parent, mode) {
  ...
  if (methods.isExist.call(gantt, id)) return;
  data.id = id;
...
gantt._delete_callback = function (upd, id, parent, mode) {
  ...
  if (methods.isExist.call(gantt, id)) methods["delete"].call(gantt, id);
};
...
gantt.init = function (node, from, to) {
  ...
  if (!this.config.scroll_size) this.config.scroll_size = this._detectScrollSize();
...
gantt._init_html_area = function (node) {
  if (typeof node == "string") this._obj = document.getElementById(node);else this._obj = node;
...
gantt.getScrollState = function () {
  if (this.$task && this.$task_data) return { x: this.$task.scrollLeft, y: this.$task_data.scrollTop };else return null;
};
...
gantt._set_scroll_events = function () {
  ...
  function onMouseWheel(e) {
    ...
    if (wx && Math.abs(wx) > Math.abs(wy)) {
      ...
      if (!gantt.config.prevent_default_scroll && gantt._cached_scroll_pos && (gantt._cached_scroll_pos.y == top || gantt._cached_scroll_pos.y <= 0 && top <= 0)) return true;
  ...
  if (ff) this.event(gantt.$container, "wheel", onMouseWheel);else this.event(gantt.$container, "mousewheel", onMouseWheel);
};
...
gantt.selectTask = function (id) {
  ...
  if (id) {
    ...
    if (this._selected_task == id) return this._selected_task;
...
gantt.ajax = {
  ...
  parse: function parse(data) {
    ...
    var obj;
    data = data.replace(/^[\s]+/, "");
    if (window.DOMParser && !gantt.env.isIE) {
      // ff,ie9
  ...
  query: function query(config) {
    gantt.ajax._call(config.method || "GET", config.url, config.data || "", config.async || true, config.callback, null, config.headers);
  },
  get: function get(url, onLoad) {
    this._call("GET", url, null, true, onLoad);
  },
  getSync: function getSync(url) {
    return this._call("GET", url, null, false);
  },
  put: function put(url, postData, onLoad) {
    this._call("PUT", url, postData, true, onLoad);
  },
  del: function del(url, postData, onLoad) {
    this._call("DELETE", url, postData, true, onLoad);
  },
  post: function post(url, postData, onLoad) {
  ...
  postSync: function postSync(url, postData) {
    postData = postData === null ? "" : String(postData);
    return this._call("POST", url, postData, false);
  },
  getLong: function getLong(url, onLoad) {
    this._call("GET", url, null, true, onLoad, { url: url });
  },
  postLong: function postLong(url, postData, onLoad) {
    if (arguments.length == 2 && (typeof postData == "function" || _typeof(window[postData]))) {
      onLoad = postData;
      postData = "";
    }
    this._call("POST", url, postData, true, onLoad, { url: url, postData: postData });
  },
  _call: function _call(method, url, postData, async, onLoad, longParams, headers) {
...
(function () {
  ...
  if (document.attachEvent) document.attachEvent("onkeydown", modal_key);else document.addEventListener("keydown", modal_key, true);
  ...
  function _boxStructure(config, ok, cancel) {
    ...
    if (ok) inner += button(getFirstDefined(config.ok, gantt.locale.labels.message_ok, "OK"), true);
    if (cancel) inner += button(getFirstDefined(config.cancel, gantt.locale.labels.message_cancel, "Cancel"), false);
    if (config.buttons) {
  ...
  function _createBox(config, ok, cancel) {
    ...
    if (config.hidden) gantt.modalbox.hide(box);
  ...
  function params(text, type, expire, id) {
    if ((typeof text === "undefined" ? "undefined" : _typeof(text)) != "object") text = { text: text, type: type, expire: expire, id: id };
  ...
  parseDate: function parseDate(date, format) {
    ...
    if (date && !date.getFullYear) {
      if (gantt.defined(format)) {
        if (typeof format == "string") format = gantt.defined(gantt.templates[format]) ? gantt.templates[format] : gantt.date.str_to_date(format);else format = gantt.templates.xml_date;
      }
      if (date) date = format(date);else date = null;
    }
    return date;
  }
};
          ...
          if (this && this.getAttribute) {
            if (!this.getAttribute("dhxgantt")) {
              for (var key in config) {
                if (key != "data") gantt.config[key] = config[key];
              }gantt.init(this);
              if (config.data) gantt.parse(config.data);
...
gantt.locale = {
  date: {
    ...
    month_full: ["一月份", "二月份", "三月份", "四月份", "五月份", "六月份", "七月份", "八月份", "九月份", "十月份", "十一月份", "十二月份"],
...
gantt._touch_events = function (names, accessor, ignore) {
  ...
  gantt.event(this.$container, "contextmenu", function (e) {
    if (action_mode) return block_action(e);
  });
...
(function () {
  ...
  function deprecated(badCode, goodCode) {
    ...
    var message = [formatting, "\"", badCode, "\"", formatting, " has been deprecated in dhtmlxGantt v4.0 and will stop working in v5.0. Use ", formatting, "\"", goodCode, "\"", formatting, " instead. \nSee more details at http://docs.dhtmlx.com/gantt/migrating.html "].join("");
  ...
  var dhtmlxMethods = ["message", "alert", "confirm", "modalbox", "uid", "copy", "mixin", "defined", "bind", "assert"];
